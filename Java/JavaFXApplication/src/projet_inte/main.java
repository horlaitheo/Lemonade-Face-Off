    ...
    public static void main(String[] args) throws Exception {
       ...
       for (int i = 0; i < composantCarte.getJSONObject("map").getJSONArray("ranking").length(); i++) {
            for(int j =0; j < composantCarte.getJSONObject("map").getJSONObject("itemsByPlayer").getJSONArray(composantCarte.getJSONObject("map").getJSONArray("ranking").getString(i)).length();j++){
                  ...
                  recette.add(new Recette( composantCarte.getJSONObject("map").getJSONObject("drinksByPlayer").getJSONArray(composantCarte.getJSONObject("map").getJSONArray("ranking").getString(j)).getJSONObject(i).getString("name")  ,  composantCarte.getJSONObject("map").getJSONObject("drinksByPlayer").getJSONArray(composantCarte.getJSONObject("map").getJSONArray("ranking").getString(j)).getJSONObject(i).getDouble("price")  ) );
            }
      ...
      carte.NombreClient();
        ...
        //System.out.println(composantCarte.getJSONObject("map").getJSONArray("ranking").getString(0));
